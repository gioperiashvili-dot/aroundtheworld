...
async function getFirebaseAuthUser(uid) {
  try {
    return await admin.auth(getFirebaseAdminApp()).getUser(uid);
  } catch (error) {
    if (error.code === "auth/user-not-found") {
      throw createBookingError(
        404,
        "Firebase Auth user not found",
        "Choose an existing registered user before creating a custom booking.",
        "AUTH_USER_NOT_FOUND"
      );
    }
...
async function createMissingUserProfileIfNeeded(db, uid, authUser, profileSnapshot) {
  ...
  const providerIds = getProviderIds(authUser);
  const displayName = normalizeText(authUser.displayName, 300);
  const email = normalizeText(authUser.email, 320);
  ...
  await db.runTransaction(async (transaction) => {
    ...
    transaction.create(profileRef, {
      ...
      photoURL: normalizeText(authUser.photoURL, 1000),
      phone: "",
      authProvider: providerIds.join(","),
      emailVerified: authUser.emailVerified === true,
      phoneVerified: false,
      createdAt: timestamp,
      updatedAt: timestamp,
      createdFromAdminSync: true,
    });
  });
...
function normalizeCustomBookingPayload(payload = {}) {
  const uid = normalizeText(payload.uid, 200);
  const title = normalizeText(payload.title, 300);
  const category =
    normalizeText(payload.category || payload.serviceType, 80) || "custom-service";
  const serviceType =
    normalizeText(payload.serviceType || payload.category, 80) || category;
  const status = normalizeText(payload.status || "active", 40).toLowerCase();
  const totalPrice = normalizeNumber(payload.totalPrice);
  const paidAmountText = String(payload.paidAmount ?? "").trim();
  const paidAmount = paidAmountText ? normalizeNumber(payload.paidAmount) : 0;
  ...
  if (!uid) {
    throw createBookingError(
      ...
      "Choose a registered user before creating a custom booking.",
      "BOOKING_UID_REQUIRED"
    );
  }
  ...
  if (!title) {
    throw createBookingError(
      ...
      "Enter a title before creating a custom booking.",
  ...
  if (!currency) {
    throw createBookingError(
      ...
      "Enter a currency before creating a custom booking.",
...
function mapBooking(documentSnapshot) {
  ...
  return {
    ...
    source: data.source || "",
    isCustomBooking: data.isCustomBooking === true,
...
async function createCustomBooking(payload = {}) {
  const bookingPayload = normalizeCustomBookingPayload(payload);
  const authUser = await getFirebaseAuthUser(bookingPayload.uid);
  const db = getAdminFirestore();
  const profileRef = db.collection("users").doc(bookingPayload.uid);
  const initialProfileSnapshot = await profileRef.get();
  const profileSnapshot = await createMissingUserProfileIfNeeded(
    db,
    bookingPayload.uid,
    authUser,
  ...
  const phone = normalizeText(profileData.phone || authUser.phoneNumber, 120);

  await bookingRef.set({
    uid: bookingPayload.uid,
    customerEmail,
    customerName,
    phone,
    source: "admin_custom",
  ...
  return {
    ...
    profileCreatedFromAdminSync:
      !initialProfileSnapshot.exists && profileSnapshot.exists,
  };
}

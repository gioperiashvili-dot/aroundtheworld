const { getPublicTours } = require("./tours");
...
function addEntry(entriesByLoc, entry) {
  ...
  if (!loc || entriesByLoc.has(loc)) {
    return;
  }

  entriesByLoc.set(loc, buildUrlXml(entry));
}

async function buildSitemapXml() {
  const [tours, blogs] = await Promise.all([getPublicTours(), getPublishedBlogs()]);
  ...
  blogs
    ...
    .forEach((blog) => {
      addEntry(entriesByLoc, {
        ...
        lastmod: formatLastmod(blog.publishedAt || blog.updatedAt || blog.createdAt),
        changefreq: "weekly",
        priority: "0.7",
      });
    });
...
${Array.from(entriesByLoc.values()).join("\n")}
</urlset>
`;
}

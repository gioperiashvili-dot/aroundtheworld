...
const { getBookingEmailErrorDiagnostics } = require("../services/bookingEmail");
const {
  buildErrorResponse,
  ...
  try {
    ...
    logProviderDiagnostic(
      ...
      "booking request email failed",
      {
        ...
        emailError: getBookingEmailErrorDiagnostics(error),
      },
...
router.get("/:idOrSlug", async (req, res) => {
  try {
    const tour = await getPublicTourByIdOrSlug(req.params.idOrSlug);

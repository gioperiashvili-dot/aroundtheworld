...
import { getHotelImageSources, normalizeTourImageSources } from "../lib/tourImages";
...
function normalizeHotelImages(hotel) {
  return normalizeTourImageSources(
    getHotelImageSources(hotel).map(resolvePublicAssetUrl)
  );
}
...
function HotelOptionsModal({ hotels, text, onClose, onImageOpen }) {
  ...
  return createPortal(
    <div
      ...
      className="fixed inset-0 z-[1000] flex items-center justify-center overflow-y-auto bg-black/76 px-3 py-4 backdrop-blur-md sm:px-4 sm:py-6"
    >
      <button
        type="button"
      ...
      <div className="relative z-10 flex max-h-[calc(100vh-2rem)] w-full max-w-6xl flex-col overflow-hidden rounded-[1.25rem] border border-white/12 bg-[#151515] text-white shadow-[0_34px_120px_-48px_rgba(0,0,0,0.95)] sm:max-h-[calc(100vh-3rem)]">
...
function HotelRating({ mealPlan, rating, reviewCount, text }) {
  return (
    <div className="flex flex-wrap items-center gap-2 rounded-[0.85rem] border border-[rgba(245,184,0,0.2)] bg-[rgba(245,184,0,0.08)] p-2.5">
      ...
      {SHOW_PUBLIC_REVIEWS && reviewCount !== null ? (
        <span className="text-sm font-semibold text-white/72">
          {text.reviews} ({reviewCount.toLocaleString()})
        </span>
      ) : null}
      {SHOW_PUBLIC_REVIEWS && rating !== null ? (

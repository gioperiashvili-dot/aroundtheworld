...
function getCalendarPriceClassName(priceTier, isSelected) {
  if (isSelected) {
    return "flight-calendar-price-badge--selected";
  }

  if (priceTier === "cheap") {
    return "flight-calendar-price-badge--cheap";
  }

  if (priceTier === "expensive") {
    return "flight-calendar-price-badge--expensive";
  }

  return "flight-calendar-price-badge--medium";
}
          ...
          <form className="space-y-5" onSubmit={handleSubmit}>
            ...
            <div className="flex flex-col gap-3 xl:flex-row xl:items-stretch">
              ...
              <button
                ...
                className={
                  isBookingVariant
                    ? "booking-field-surface flex h-12 w-12 shrink-0 items-center justify-center self-center rounded-full border text-[var(--aw-input-muted)] transition hover:-translate-y-0.5 hover:border-[var(--aw-accent)] hover:text-[var(--aw-input-text)] xl:h-[4.8rem] xl:w-[4.8rem]"
                    : "flex h-12 w-12 shrink-0 items-center justify-center self-center rounded-full border border-slate-200 bg-white text-slate-700 shadow-[0_16px_34px_-26px_rgba(15,23,42,0.8)] transition hover:-translate-y-0.5 hover:border-[var(--aw-accent)] hover:text-slate-950 dark:border-white/10 dark:bg-[var(--aw-panel)] dark:text-white/84 dark:hover:border-[var(--aw-accent)] dark:hover:text-[var(--aw-accent-hover)] xl:h-[4.6rem] xl:w-[4.6rem]"
                }
              >
                <SwapIcon />
              </button>
              ...
              <button
                ...
                className={
                  isBookingVariant
                    ? "booking-search-submit aw-premium-button group flex h-14 w-full shrink-0 items-center justify-center rounded-[1rem] px-6 text-sm font-black uppercase text-slate-950 transition hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:bg-slate-600 disabled:text-slate-300 disabled:shadow-none xl:h-[4.8rem] xl:w-auto xl:min-w-[10rem]"
            ...
            <div
              className={
                isBookingVariant
                  ? "flex flex-col gap-4 rounded-[1.15rem] border border-white/12 bg-[#031015]/28 p-4 md:flex-row md:items-center md:justify-between"
            ...
            >
              <label
                className={`flex items-center gap-3 text-sm font-medium ${
                  isBookingVariant ? "text-white/82" : "text-slate-700 dark:text-slate-200"
                }`}
              >
                <input
                  type="checkbox"
                ...
                {QUICK_DESTINATIONS.map((code) => {
                  ...
                  return (
                    <button
                      ...
                      className={`rounded-full bg-[var(--aw-accent-soft)] px-3 py-2 text-sm font-semibold transition hover:bg-[rgba(246,196,69,0.24)] ${
                        isBookingVariant
...
function TripTypeOptions({
  ...
  return (
    <div
      ...
      className={
        isBookingVariant
          ? "inline-flex w-full flex-wrap gap-1 rounded-full bg-[#031015]/38 p-1 sm:w-auto"
        ...
        return (
          <button
            ...
            className={`min-h-10 flex-1 rounded-full px-4 py-2 text-sm font-semibold transition sm:flex-none ${
              isBookingVariant
                ? isSelected
                  ? "bg-[var(--aw-accent)] text-slate-950 shadow-[0_12px_30px_-22px_rgba(246,196,69,0.9)]"
  ...
  return (
    <div
      className={`min-h-[4.6rem] min-w-0 px-4 py-3 transition ${
        isBookingVariant
          ? "booking-field-surface rounded-[1rem] border hover:-translate-y-0.5 focus-within:-translate-y-0.5 focus-within:border-[var(--aw-accent)]"
  ...
  return (
    <div className="relative text-left">
      ...
      {isOpen && typeof document !== "undefined" ? createPortal(
        <div
          className="flight-calendar-overlay fixed inset-0 z-[1000] flex items-end justify-center bg-black/55 p-0 backdrop-blur-sm sm:items-center sm:p-6"
          role="dialog"
          aria-modal="true"
        >
          <button
          ...
          <div className="flight-calendar-dialog__header flex items-center justify-between gap-3 border-b border-slate-200 pb-3">
            <button
              type="button"
              onClick={() => setVisibleMonth((currentMonth) => addMonths(currentMonth, -1))}
              className="flight-calendar-nav-button flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-700 shadow-sm transition hover:border-[var(--aw-accent)] hover:bg-[rgba(245,184,0,0.08)] hover:text-slate-950"
          ...
          <div className="flight-calendar-months mt-4 grid gap-5 md:grid-cols-2">
            {[visibleMonth, addMonths(visibleMonth, 1)].map((monthDate, index) => (
              <div
                key={`${monthDate.getFullYear()}-${monthDate.getMonth()}`}
                className={`flight-calendar-month-shell ${index === 1 ? "hidden md:block" : ""}`}
              >
                <CalendarMonth
                  monthDate={monthDate}
                  selectedValue={value}
          ...
          {priceCalendarLoading || priceCalendarError || priceCalendarCurrencyFallback ? (
            <div className="flight-calendar-status mt-4 rounded-[0.85rem] bg-white px-3 py-2 text-center text-xs font-semibold text-slate-500">
          ...
          {showNoReturnAction ? (
            <button
              ...
              className="flight-calendar-action mt-4 w-full rounded-[0.9rem] border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-slate-700 transition hover:border-[var(--aw-accent)] hover:text-slate-950"
            >
...
function CalendarMonth({
  ...
  return (
    <section className="flight-calendar-month">
      ...
      <div className="flight-calendar-grid mt-4 grid grid-cols-7 gap-1 text-center">
        {weekdayLabels.map((weekday) => (
          <span
            key={weekday}
            className="calendar-weekday flight-calendar-weekday py-1 text-[11px] font-bold text-slate-500"
          >
            {weekday}
          </span>
        ))}
        {getMonthDays(monthDate).map((date, index) => {
          if (!date) {
            return <span key={`blank-${index}`} className="flight-calendar-day-placeholder min-h-11" />;
          }
          ...
          return (
            <button
              ...
              className={`flight-calendar-day flex min-h-[3.75rem] flex-col items-center justify-center rounded-[0.8rem] border px-1.5 py-2 text-sm font-bold transition ${
                isSelected
                  ? "flight-calendar-day--selected border-[var(--aw-accent)] bg-[var(--aw-accent)] text-slate-950 shadow-[0_14px_30px_-20px_rgba(245,184,0,0.95)]"
                  : isToday
                    ? "flight-calendar-day--today border-[var(--aw-accent)] bg-[rgba(245,184,0,0.08)] text-slate-950"
                    : "flight-calendar-day--default border-transparent bg-transparent text-slate-700 hover:border-[rgba(245,184,0,0.42)] hover:bg-[rgba(245,184,0,0.08)] hover:text-slate-950"
              }`}
            >
              <span className="flight-calendar-day__number">{date.getDate()}</span>
              {priceLabel ? (
                <span
                  className={`flight-calendar-price-badge mt-1 max-w-full rounded-full px-1.5 py-0.5 text-[10px] font-black leading-none tabular-nums sm:text-[11px] ${getCalendarPriceClassName(
                    priceTier,
                    isSelected
                  )}`}
                >
  ...
  return (
    <div className="mt-4 flex flex-wrap justify-center gap-2">
      {items.map((item) => (
        <span
          key={item.key}
          className={`flight-calendar-price-badge inline-flex items-center rounded-full px-2.5 py-1 text-[11px] font-bold ${getCalendarPriceClassName(
            item.tier,
            false
          )}`}
        >
  ...
  return (
    <div
      className={`min-h-[4.6rem] px-4 py-3 text-left ${
        isBookingVariant
          ? "booking-field-surface rounded-[1rem] border transition hover:-translate-y-0.5 focus-within:-translate-y-0.5 focus-within:border-[var(--aw-accent)]"
  ...
  return (
    <div className={`relative ${className}`}>
      <button
        ...
        className={
          isBookingVariant
            ? "booking-field-surface min-h-[4.8rem] w-full rounded-[1rem] border px-4 py-3 text-left transition hover:-translate-y-0.5 hover:border-[var(--aw-accent)]"

...
export default function BookingSearchTabs({
  ...
  return (
    <section
      className={`booking-search-tabs aw-glass-panel overflow-visible rounded-[1.55rem] ${className}`}
    >
      <div className="booking-search-tabs__inner relative z-10 space-y-5 p-4 sm:p-5 lg:p-6 xl:p-7">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div
            role="tablist"
            aria-label={t("booking.tabsLabel")}
            className="booking-tab-list flex gap-2 overflow-x-auto rounded-full border border-white/10 bg-[#031015]/42 p-1.5"
          >
              ...
              return (
                <button
                  ...
                  className={`shrink-0 rounded-full border px-4 py-2 text-sm font-black uppercase transition ${
                    isSelected
                      ? "border-[rgba(246,196,69,0.32)] bg-[rgba(246,196,69,0.95)] text-slate-950 shadow-[0_14px_30px_-24px_rgba(246,196,69,0.95)]"
                      : "border-white/8 bg-white/6 text-white/72 hover:border-white/16 hover:bg-white/10 hover:text-white"
                  }`}
                >
                  {t(`booking.tabs.${tab}`)}
                </button>
          ...
          <label className="flex w-full flex-col sm:w-auto sm:min-w-56">
            ...
            <select
              ...
              className="booking-field-surface min-h-12 rounded-[1rem] border px-3 text-sm font-bold outline-none transition focus:border-[var(--aw-accent)]"
            >
        ...
        {activeTab === "hotel" ? (
          <form className="space-y-5" onSubmit={handleHotelSubmit}>
            <div className="grid gap-3 lg:grid-cols-2 xl:grid-cols-[1.22fr_0.82fr_0.82fr_0.62fr_0.62fr_auto] xl:items-stretch">
              <BookingField>
                <AutocompleteInput
...
function BookingField({ children }) {
  return (
    <div className="booking-field-surface min-h-[4.8rem] rounded-[1rem] border px-4 py-3 transition hover:-translate-y-0.5 focus-within:-translate-y-0.5 focus-within:border-[var(--aw-accent)]">
      {children}
    </div>
  );
}
...
function BookingSubmitButton({ label, disabled = false }) {
  return (
    <button
      type="submit"
      disabled={disabled}
      className="booking-search-submit aw-premium-button flex min-h-[4.8rem] w-full items-center justify-center rounded-[1rem] px-7 text-sm font-black uppercase text-slate-950 transition hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:bg-slate-600 disabled:text-slate-300 disabled:shadow-none md:w-auto"
    >
      {label}
    </button>
  );

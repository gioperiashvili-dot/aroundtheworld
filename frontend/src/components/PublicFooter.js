...
export default function PublicFooter() {
  ...
  const links = [
    ...
    { label: privacyPolicyLabel, href: PRIVACY_POLICY_HREF, brand: "privacy" },
  ];

  return (
    <footer className="relative z-10 mx-auto w-full max-w-[1500px] px-4 pb-10 sm:px-6 lg:px-8">
      <div className="grid gap-5 rounded-[2rem] border border-white/70 bg-white px-6 py-6 text-slate-900 shadow-[0_26px_90px_-58px_rgba(15,23,42,0.55)] backdrop-blur-xl dark:border-white/16 dark:bg-slate-950/58 dark:text-white dark:shadow-[0_26px_90px_-58px_rgba(15,23,42,0.85)] md:grid-cols-[1fr_auto] md:items-center">
        <div className="space-y-2">
          <p className="text-sm font-medium tracking-[0.02em] text-slate-950 dark:text-white/88">
            Around The World - All Rights Reserved © {year}
          </p>
          <p className="text-sm text-slate-700 dark:text-white/72">
            {t("contact.address")}: {contactDetails.address}
          </p>
          <a
            href={getPhoneHref()}
            className="inline-flex text-sm font-semibold text-slate-800 transition hover:text-slate-950 dark:text-white/84 dark:hover:text-white"
          >
            {t("contact.phone")}: {contactDetails.phone}
          </a>
          <a
            href={getEmailHref()}
            className="block break-words text-sm font-semibold text-slate-800 transition hover:text-slate-950 dark:text-white/84 dark:hover:text-white"
          >
        ...
        <div className="flex flex-wrap gap-2 md:justify-end">
          {links.map((link) => (
            <a
              ...
              className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-slate-50 px-4 py-2 text-sm font-semibold text-slate-800 transition hover:bg-white hover:text-slate-950 dark:border-white/14 dark:bg-white/8 dark:text-white/84 dark:hover:bg-white/14 dark:hover:text-white"
            >
...
function SocialMark({ brand }) {
  return (
    <span className={getSocialMarkClass(brand)}>
      ...
      {brand === "privacy" ? <PrivacyIcon /> : null}
    </span>
  );
}
...
function getSocialMarkClass(brand) {
  ...
  if (brand === "privacy") {
    return `${baseClass} bg-slate-800 dark:bg-white dark:text-slate-950`;
  }

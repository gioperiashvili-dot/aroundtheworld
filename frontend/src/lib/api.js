...
import { normalizeBlogListResponse } from "./blogs";
...
export async function previewAdminTourImportDraft(token, payload = {}) {
  const formData = new FormData();
  const postText = String(payload.postText || "");
  ...
  const response = await apiClient.post(
    "/api/admin/tours/import-draft/preview",
    formData,
    getAdminConfig(token)
  );
...
export async function createAdminCustomBooking(token, payload) {
  const response = await apiClient.post(
    "/api/admin/bookings/custom",
    payload,
    getAdminConfig(token)
  );

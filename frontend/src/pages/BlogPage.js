...
import { getVisibleBlogPosts } from "../lib/blogs";
...
export default function BlogPage() {
    ...
    const loadReviews = async () => {
      ...
      try {
        const [reviewsResponse, toursResponse] = await Promise.all([
          fetchReviews({ limit: PUBLIC_REVIEWS_LIMIT }),
          fetchTours().catch(() => ({ tours: [] })),
        ]);
        const tours = Array.isArray(toursResponse?.tours) ? toursResponse.tours : [];
        const responseReviews = Array.isArray(reviewsResponse?.reviews)
          ? reviewsResponse.reviews
          : [];
        const responseTotal = Number(reviewsResponse?.total);

        setReviews(responseReviews.slice(0, PUBLIC_REVIEWS_LIMIT));
        setReviewTotal(
  ...
  const visiblePosts = useMemo(() => getVisibleBlogPosts(blogs), [blogs]);
      ...
      <section className="space-y-10">
        <div className="rounded-[1rem] border border-white/10 bg-[#202020] p-6 text-white shadow-[0_30px_90px_-60px_rgba(0,0,0,0.92)] lg:p-8">
          ...
          <div className="mt-6">
            ...
            {!blogsLoading && !blogsError && visiblePosts.length > 0 ? (
        ...
        {SHOW_PUBLIC_REVIEWS ? (
          <div className="rounded-[1rem] border border-white/10 bg-[#202020] p-6 text-white shadow-[0_30px_90px_-60px_rgba(0,0,0,0.92)] lg:p-8">
            <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
              ...
              {!reviewsLoading ? (
                <p className="text-sm font-semibold text-white/70">
                  {formatReviewCount(reviewTotal, language)}
                </p>
              ) : null}
            </div>
            ...
            <div className="mt-6">
              ...
              {!reviewsLoading && !reviewsError ? (
                <p className="mt-4 rounded-full border border-white/10 bg-white/[0.05] px-4 py-3 text-center text-sm font-semibold text-white/64">
                  {t("reviews.totalCountPrefix")} {formatReviewCount(reviewTotal, language)}
                </p>
              ) : null}
            </div>
          </div>

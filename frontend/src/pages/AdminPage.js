...
import AdminTourImportPanel from "../components/AdminTourImportPanel";
...
const ADMIN_USER_MESSAGES = {
  ka: {
    loadFailed: "მომხმარებლების ჩატვირთვა ვერ მოხერხდა.",
    createFailed: "ინდივიდუალური ჯავშნის შექმნა ვერ მოხერხდა.",
    createSuccess: "ინდივიდუალური ჯავშანი შეიქმნა.",
    createErrors: {
      AUTH_USER_NOT_FOUND: "Firebase Auth მომხმარებელი ვერ მოიძებნა.",
      BOOKING_UID_REQUIRED: "აირჩიეთ რეგისტრირებული მომხმარებელი.",
      BOOKING_TITLE_REQUIRED: "სათაური აუცილებელია.",
      BOOKING_STATUS_INVALID:
        "აირჩიეთ სტატუსი: აქტიური, დასრულებული ან გაუქმებული.",
      BOOKING_TOTAL_PRICE_INVALID: "ჯამური ღირებულება უნდა იყოს 0-ზე მეტი რიცხვი.",
      BOOKING_PAID_AMOUNT_INVALID:
        "გადახდილი თანხა უნდა იყოს 0 ან მეტი რიცხვი.",
      BOOKING_PAID_AMOUNT_TOO_HIGH:
        "გადახდილი თანხა ჯამურ ღირებულებას არ უნდა აღემატებოდეს.",
      BOOKING_CURRENCY_REQUIRED: "ვალუტა აუცილებელია.",
    },
  },
  en: {
    loadFailed: "Something went wrong while loading users.",
    createFailed: "Something went wrong while creating the custom booking.",
    createSuccess: "Custom booking created.",
    createErrors: {
      AUTH_USER_NOT_FOUND: "Firebase Auth user was not found.",
      BOOKING_UID_REQUIRED: "Choose a registered user.",
      BOOKING_TITLE_REQUIRED: "Title is required.",
      BOOKING_STATUS_INVALID: "Choose active, completed, or cancelled.",
      BOOKING_TOTAL_PRICE_INVALID:
        "Total price must be a number greater than 0.",
...
function toFormValues(tour) {
  ...
  return {
    ...
    seoTitleKa: tour?.seoTitle?.ka || "",
    seoTitleEn: tour?.seoTitle?.en || "",
    seoDescriptionKa: tour?.seoDescription?.ka || "",
    seoDescriptionEn: tour?.seoDescription?.en || "",
    canonicalUrl: tour?.canonicalUrl || "",
    importMeta: tour?.importMeta || null,
  };
}
...
function toHotelFormRows(hotels = []) {
  ...
  return hotels
    .map((hotel, index) => ({
      ...
      images: getHotelImageSources(hotel),
    }))
    .filter((hotel) =>
      hotel.name ||
      hotel.location ||
...
export default function AdminPage() {
  ...
  const userMessages = ADMIN_USER_MESSAGES[language] || ADMIN_USER_MESSAGES.ka;
  const adminSeo = (
  ...
  useEffect(() => {
    ...
    if (usersError) {
      setAdminToast({ type: "error", message: usersError });
    }
  }, [bookingsError, bookingRequestsError, error, success, usersError]);
  ...
  const adminTabs = useMemo(
    () => [
      ...
      {
        id: ADMIN_TAB_IDS.tourImport,
        label: adminCopy.tourImport,
      },
      {
  ...
  const loadAdminUsers = useCallback(
    async (activeToken) => {
      ...
      try {
        const response = await fetchAdminUsers(activeToken);
        ...
        setUsersError(
          getFriendlyApiError(requestError, userMessages.loadFailed, {
            unauthorizedMessage: t("admin.errors.loginFailed"),
          })
        );
      } finally {
    ...
    [clearSession, t, userMessages.loadFailed]
  );
  ...
  const validateForm = () => {
    ...
    if (!["active", "draft"].includes(status)) {
      return language === "en"
        ? "Choose a valid tour status."
        : "\u10D0\u10D8\u10E0\u10E9\u10D8\u10D4\u10D7 \u10E1\u10EC\u10DD\u10E0\u10D8 \u10E1\u10E2\u10D0\u10E2\u10E3\u10E1\u10D8.";
    }
    ...
    if ((!isDraft || String(form.price || "").trim()) && (Number.isNaN(priceValue) || priceValue < 0)) {
    ...
    if (!isDraft && !form.slug.trim()) {
      return getTourSlugRequiredMessage(language);
    }

    if (form.slug.trim() && !isValidTourSlug(form.slug)) {
  ...
  const buildPayload = (imageValues = getFormGalleryImages(form)) => {
    ...
    const priceSource = String(form.price || "").trim();
    const titleSource = getTourSlugSourceFromForm(form) || "tour";
    ...
    return {
      ...
      price: priceSource ? Number(priceSource) : null,
      ...
      slug: normalizeTourSlug(form.slug) || createTourSlug(titleSource, "tour"),
      image: galleryImages[0] || "",
      ...
      importMeta: form.importMeta || undefined,
    };
  };
  ...
  const handleCreateCustomBooking = async (payload) => {
    setUserActionId(payload.uid || "");
    setUsersError("");
    setBookingsError("");
    setError("");
    setSuccess("");
    ...
    try {
      await createAdminCustomBooking(token, payload);
      setSuccess(userMessages.createSuccess);
      await Promise.all([
      ...
      const createErrorCode = requestError.response?.data?.code;
      const localizedCreateError =
        userMessages.createErrors?.[createErrorCode];

      setUsersError(
        localizedCreateError ||
          getFriendlyApiError(requestError, userMessages.createFailed, {
            unauthorizedMessage: t("admin.errors.loginFailed"),
          })
      );
      return false;
    ...
    <div className="min-h-screen bg-[#30302f] px-3 py-5 text-slate-900 dark:bg-slate-950 dark:text-slate-100 sm:px-5 sm:py-8">
      ...
      <main className="mx-auto max-w-[1540px] overflow-hidden rounded-[2.9rem] border-[8px] border-white/10 bg-[#fff] p-4 shadow-[0_34px_120px_-54px_rgba(0,0,0,0.8)] dark:border-white/10 dark:bg-slate-900/90 sm:p-5 lg:p-6">
        ...
        <section className="mt-7 space-y-6">
          ...
          <AdminTabPanel active={activeAdminTab === ADMIN_TAB_IDS.tourImport}>
            <AdminTourImportPanel
              token={token}
              onUnauthorized={clearSession}
              onSaved={() => {
                setSuccess(
                  language === "en"
                    ? "Draft created. Review it in Packages before publishing."
                    : "\u10D3\u10E0\u10D0\u10E4\u10E2\u10D8 \u10E8\u10D4\u10D8\u10E5\u10DB\u10DC\u10D0. \u10D2\u10D0\u10D3\u10D0\u10D0\u10DB\u10DD\u10EC\u10DB\u10D4 \u10E2\u10E3\u10E0\u10D4\u10D1\u10E8\u10D8 \u10D2\u10D0\u10DB\u10DD\u10E5\u10D5\u10D4\u10E7\u10DC\u10D4\u10D1\u10D0\u10DB\u10D3\u10D4."
                );
                void loadAdminTours(token);
              }}
            />
          ...
          <AdminTabPanel active={activeAdminTab === ADMIN_TAB_IDS.users}>
            <AdminUsersPanel
              users={users}
              loading={usersLoading}
              error={usersError}
              actionId={userActionId}
              onCreateBooking={handleCreateCustomBooking}
            />
          </AdminTabPanel>
              ...
              {tours.map((tour) => {
                ...
                return (
                  ...
                  >
                    ...
                    {tour.status === "draft" ? (
                      <span className="absolute left-4 top-4 rounded-full bg-amber-50 px-3 py-1 text-xs font-bold uppercase tracking-[0.18em] text-amber-800 shadow-lg shadow-black/10">
                        Draft
                      </span>
                    ) : null}
